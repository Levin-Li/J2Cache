...
public class SpringRedisPubSubPolicy implements ClusterPolicy {
    ...
    private J2CacheBootConfig config;
    ...
    public void connect(Properties props, CacheProviderHolder holder) {
        this.holder = holder;
        this.config = SpringBeanHolder.instance.getBootConfig();
        if (config.getL2CacheOpen() == false) {
            return;
        }
        J2CacheConfig j2config = SpringBeanHolder.instance.getConfig();
        this.redisTemplate = SpringBeanHolder.instance.getRedisTemplate();
        String channel_name = j2config.getL2CacheProperties().getProperty("channel");
        if (channel_name != null && !channel_name.isEmpty()) {
            this.channel = channel_name;
        }
        RedisMessageListenerContainer listenerContainer = SpringBeanHolder.instance.getListenerContainer();
