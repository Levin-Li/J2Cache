...
public final class Timestamper {
    ...
    public static final int BIN_DIGITS = Integer.getInteger("net.oschina.j2cache.hibernate5.redis.util.Timestamper.shift", 12);

    public static final int ONE_MS = 1 << BIN_DIGITS;

    private static final Logger LOG     = LoggerFactory.getLogger(Timestamper.class);
    private static final int    MAX_LOG = Integer.getInteger("net.oschina.j2cache.hibernate5.redis.util.Timestamper.log.max", 1) * 1000;
                ...
                if (VALUE.compareAndSet(current, update)) {
                    if (runs > 1) {
                        log(base, "Thread spin-waits on time to pass. Looped "
                                + "{} times, you might want to increase -Dnet.oschina.j2cache.hibernate5.redis.util.Timestamper.shift", runs);
                    }
                    return update;
                }
            }
